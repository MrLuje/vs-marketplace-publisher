...
async function run() {
  ...
  async function publishToMarketplace(vsixPath: string, manifestPath: string, personalAccessToken: string): Promise<boolean> {
    ...
    core.info("Publishing package to marketplace...");
    ...
    core.info("Successfully published package to marketplace !");
  ...
  async function getLatestReleaseFile() {
    ...
    core.info(`Using assets from release ${release!.id}`);
    ...
    core.info(`Downloading package ${vsixAsset.name}`);
    ...
    core.info(`Using package ${vsixAsset.name}`);
  ...
  async function getPackageFilePath(inputs: inputs) {
    ...
    core.info(`Using ${inputs.vsixPath} as package`);
